...
async function signIn(
  email: string,
  password: string
): Promise<User | false | string | any> {
  // Retrieve user based on email
  const paramsFindUser = {
  ...
  try {
    const result = await db.query(paramsFindUser).promise();
    ...
    //   user.hashedPassword
    // );
    // return user.hashedPassword + " " + isPasswordValid;
...
async function emailExists(email: string): Promise<boolean> {
  ...
  try {
    const result = await db.scan(paramsFindUser).promise();
